...
local switch_to_agent =
    "Please use `agents` table and switch agents in runtime via `:GpAgent XY`"
local deprecated = {
...
_H.delete_buffer = function(file_name)
    ...
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
        if vim.api.nvim_buf_is_valid(b) and vim.api.nvim_buf_get_name(b) ==
            file_name then vim.api.nvim_buf_delete(b, {force = true}) end
    end
end
...
_H.create_augroup = function(name, opts)
    return vim.api.nvim_create_augroup(name .. "_" .. _H.uuid(),
                                       opts or {clear = true})
end
...
_H.undojoin = function(buf)
    ...
    if not status then
        if result:match("E790") then return end
...
_H.process = function(buf, cmd, args, callback, out_reader, err_reader)
    ...
    local on_exit = _H.once(vim.schedule_wrap(
    ...
    vim.loop.read_start(stdout, function(err, data)
        if err then M.error("Error reading stdout: " .. vim.inspect(err)) end
        if data then stdout_data = stdout_data .. data end
        if out_reader then out_reader(err, data) end
    end)

    vim.loop.read_start(stderr, function(err, data)
        if err then M.error("Error reading stderr: " .. vim.inspect(err)) end
        if data then stderr_data = stderr_data .. data end
        if err_reader then err_reader(err, data) end
    end)
end
...
_H.grep_directory = function(buf, directory, pattern, callback)
    ...
    _H.process(buf, "grep", {"-irEn", "--null", pattern, directory},
               function(c, _, stdout, _)
        local results = {}
        if c ~= 0 then
            callback(results, re)
            return
...
_H.create_popup = function(buf, title, size_func, opts, style)
    ...
    local close = _H.once(function()
        vim.schedule(function()
            ...
            if not opts.gid then vim.api.nvim_del_augroup_by_id(pgid) end
            if win and vim.api.nvim_win_is_valid(win) then
                vim.api.nvim_win_close(win, true)
            end
            if opts.persist then return end
    ...
    if opts.escape then
        ...
        _H.set_keymap({buf}, {"n", "v", "i"}, "<C-c>", close,
...
_H.last_content_line = function(buf)
    ...
    while line > 0 do
        ...
        if content:match("%S") then return line end
        line = line - 1
    end
    return 0
end
...
_H.template_replace = function(template, key, value)
    ...
    if value == nil then return template:gsub(key, "") end
...
_H.starts_with = function(str, start) return str:sub(1, #start) == start end
...
M._log = function(msg, kind, history)
    vim.schedule(function()
        vim.api.nvim_echo({{M._Name .. ": " .. msg, kind}}, history, {})
    end)
end

-- nicer error messages using nvim_echo
---@param msg string # error message
M.error = function(msg) M._log(msg, "ErrorMsg", true) end

-- nicer warning messages using nvim_echo
---@param msg string # warning message
M.warning = function(msg) M._log(msg, "WarningMsg", true) end

-- nicer plain messages using nvim_echo
---@param msg string # plain message
M.info = function(msg) M._log(msg, "Normal", true) end
...
_H.find_git_root = function()
    ...
    while cwd ~= "/" do
        ...
        if vim.tbl_contains(files, ".git") then return cwd end
...
M.repo_instructions = function()
    ...
    if vim.fn.filereadable(instruct_file) == 0 then return "" end
...
M.append_selection = function(params, origin_buf, target_buf)
    -- prepare selection
    local lines = vim.api.nvim_buf_get_lines(origin_buf, params.line1 - 1,
                                             params.line2, false)
    local selection = table.concat(lines, "\n")
    if selection ~= "" then
        local filetype = M._H.get_filetype(origin_buf)
        local fname = vim.api.nvim_buf_get_name(origin_buf)
        local rendered = M.template_render(M.config.template_selection, "",
                                           selection, filetype, fname)
        if rendered then selection = rendered end
    end
...
function M.refresh_copilot_bearer()
    if not M.providers.copilot or not M.providers.copilot.secret then return end
    local secret = M.providers.copilot.secret

    if type(secret) == "table" then return end
    ...
    local args = {
        "-s", "-v", "https://api.github.com/copilot_internal/v2/token", "-H",
        "Content-Type: application/json", "-H", "accept: */*", "-H",
        "authorization: token " .. secret, "-H",
        "editor-version: vscode/1.90.2", "-H",
        "editor-plugin-version: copilot-chat/0.17.2024062801", "-H",
        "user-agent: GitHubCopilotChat/0.17.2024062801"
    }

    for _, arg in ipairs(args) do table.insert(curl_params, arg) end

    M._H.process(nil, "curl", curl_params,
                 function(code, signal, stdout, stderr)
        if code ~= 0 then
            M.error(string.format("Copilot bearer resolve exited: %d, %d", code,
                                  signal, stderr))
            return
        end
...
M.setup = function(opts)
    ...
    for k, v in pairs(opts) do
        if deprecated[k] then
            table.insert(M._deprecated,
                         {name = k, msg = deprecated[k], value = v})
        else
            M.config[k] = v
        end
    end
    ...
    if #M._deprecated > 0 then
        ...
        table.sort(M._deprecated, function(a, b) return a.msg < b.msg end)
    ...
    for name, agent in pairs(M.agents) do
        ...
        elseif not agent.model or not agent.system_prompt then
            M.warning(
                "Agent " .. name .. " is missing model or system_prompt\n" ..
    ...
    for name, agent in pairs(M.agents) do
        ...
        if M.providers[M.agents[name].provider] then
            ...
            if agent.chat then table.insert(M._chat_agents, name) end
        else
            M.agents[name] = nil
        end
    end
    ...
    for hook, _ in pairs(M.hooks) do
        vim.api.nvim_create_user_command(M.config.cmd_prefix .. hook, function(
            params) M.call_hook(hook, params) end, {
            nargs = "?",
            range = true,
    ...
    for cmd, _ in pairs(M.cmd) do
        if M.hooks[cmd] == nil then
            vim.api.nvim_create_user_command(M.config.cmd_prefix .. cmd,
                                             function(params)
                M.cmd[cmd](params)
            end, {
                nargs = "?",
    ...
    for name, _ in pairs(M.providers) do M.resolve_secret(name) end
...
function M.resolve_secret(provider, callback)
    local post_process = function()
        ...
        if provider == "copilot" then M.refresh_copilot_bearer() end
    ...
    -- backwards compatibility
    if provider == "openai" then
        M.providers[provider].secret = M.providers[provider].secret or
    ...
    if secret and type(secret) == "table" then
        ...
        _H.process(nil, cmd, args,
            ...
            if code == 0 then
                ...
                M.warning(
                    "config.providers." .. provider .. ".secret command " ..
                        vim.inspect(secret) ..
...
M.valid_api_key = function()
    ...
    if type(api_key) == "table" then
        M.error("openai_api_key is still an unresolved command: " ..
                    vim.inspect(api_key))
        return false
    end

    if api_key and string.match(api_key, "%S") then return true end

    M.error("config.openai_api_key is not set: " .. vim.inspect(api_key) ..
                " run :checkhealth gp")
    return false
end
...
M.refresh_state = function()
    ...
    M._state.command_agent = M._state.command_agent or state.command_agent or
                                 nil
    ...
    if not M._state.image_agent == nil or
        not M.image_agents[M._state.image_agent] then
...
M.Target = {
    ...
    new = function(filetype) return {type = 5, filetype = filetype} end,

    --- for creating a new vertical split
    ---@param filetype nil | string # nil = same as the original buffer
    ---@return table # a table with type=6 and filetype=filetype
    vnew = function(filetype) return {type = 6, filetype = filetype} end,

    --- for creating a new tab
    ---@param filetype nil | string # nil = same as the original buffer
    ---@return table # a table with type=7 and filetype=filetype
    tabnew = function(filetype) return {type = 7, filetype = filetype} end
}
...
M.prepare_commands = function()
    for name, target in pairs(M.Target) do
        ...
        if target == M.Target.popup then agent = M.get_chat_agent() end
...
M.call_hook = function(name, params)
    if M.hooks[name] ~= nil then return M.hooks[name](M, params) end
    ...
    if provider == "googleai" then
        for i, message in ipairs(messages) do
            ...
            if message.content then
                messages[i].parts = {{text = message.content}}
...
function M.cleanup_old_queries(N, age)
    ...
    local query_count = 0
    for _ in pairs(M._queries) do query_count = query_count + 1 end

    if query_count <= N then return end
...
M.query = function(buf, provider, payload, handler, on_exit, callback)
    ...
    if type(handler) ~= "function" then
        M.error(string.format(
                    "query() expects a handler function, but got %s:\n%s",
                    type(handler), vim.inspect(handler)))
        return
    end
    ...
    local out_reader = function()
        ...
        local function process_lines(lines_chunk)
            ...
            for _, line in ipairs(lines) do
                ...
                line = line:gsub("^data: ", "")
                local content = ""
                if line:match("choices") and line:match("delta") and
                    line:match("content") then
                    line = vim.json.decode(line)
                    if line.choices[1] and line.choices[1].delta and
                        line.choices[1].delta.content then
                        content = line.choices[1].delta.content
                    end
                end

                if qt.provider == "anthropic" and line:match('"text":') then
                    if line:match("content_block_start") or
                        line:match("content_block_delta") then
                        line = vim.json.decode(line)
        ...
        return function(err, chunk)
            ...
            if err then
                M.error(qt.provider .. " query stdout error: " ..
                            vim.inspect(err))
            elseif chunk then
                ...
                if #buffer > 0 then process_lines(buffer) end
                ...
                if type(on_exit) == "function" then
                    ...
                    if qt.ns_id and qt.buf then
                        vim.schedule(function()
                            vim.api.nvim_buf_clear_namespace(qt.buf, qt.ns_id,
                                                             0, -1)
                        end)
                    end
                end
    ...
    if provider == "copilot" then
        ...
        headers = {
            "-H", "editor-version: vscode/1.85.1", "-H",
    ...
    if provider == "azure" then
        headers = {"-H", "api-key: " .. bearer}
        endpoint = M._H.template_replace(endpoint, "{{model}}", payload.model)
    end

    local curl_params = vim.deepcopy(M.config.curl_params or {})
    local args = {
        "--no-buffer", "-s", endpoint, "-H", "Content-Type: application/json",
    ...
    for _, header in ipairs(headers) do table.insert(curl_params, header) end
...
M.create_handler = function(buf, win, line, first_undojoin, prefix, cursor)
    ...
    return vim.schedule_wrap(function(qid, chunk)
        ...
        if not qt.ns_id then qt.ns_id = ns_id end

        if not qt.ex_id then qt.ex_id = ex_id end

        first_line =
            vim.api.nvim_buf_get_extmark_by_id(buf, ns_id, ex_id, {})[1]

        -- clean previous response
        local line_count = #vim.split(response, "\n")
        vim.api.nvim_buf_set_lines(buf, first_line + finished_lines,
                                   first_line + line_count, false, {})
        ...
        for i, l in ipairs(lines) do lines[i] = prefix .. l end
        ...
        vim.api.nvim_buf_set_lines(buf, first_line + finished_lines,
                                   first_line + finished_lines, false,
                                   unfinished_lines)

        local new_finished_lines = math.max(0, #lines - 1)
        for i = finished_lines, new_finished_lines do
            vim.api.nvim_buf_add_highlight(buf, qt.ns_id, hl_handler_group,
                                           first_line + i, 0, -1)
        end
        ...
        if cursor then M._H.cursor_to_line(end_line, buf, win) end
    end)
end
...
M._toggle_close = function(kind)
    if M._toggle[kind] and M._toggle[kind].win and M._toggle[kind].buf and
        M._toggle[kind].close and vim.api.nvim_win_is_valid(M._toggle[kind].win) and
        vim.api.nvim_buf_is_valid(M._toggle[kind].buf) and
        vim.api.nvim_win_get_buf(M._toggle[kind].win) == M._toggle[kind].buf then
...
M._toggle_add = function(kind, toggle) M._toggle[kind] = toggle end
...
M.prep_md = function(buf)
    ...
    vim.api.nvim_command("autocmd TextChanged,InsertLeave <buffer=" .. buf ..
                             "> silent! write")
...
M.not_chat = function(buf, file_name)
    ...
    if not _H.starts_with(file_name, chat_dir) then
        return "resolved file (" .. file_name .. ") not in chat dir (" ..
                   chat_dir .. ")"
    end

    local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
    if #lines < 7 then return "file too short" end

    if not lines[1]:match("^# ") then return "missing topic header" end
    ...
    if not header_found then return "missing file header" end
...
M.prep_chat = function(buf, file_name)
    ...
    for _, rc in ipairs(range_commands) do
        ...
        for _, mode in ipairs(rc.modes) do
            if mode == "n" or mode == "i" then
                ...
                _H.set_keymap({buf}, mode, rc.shortcut, ":<C-u>'<,'>" .. cmd,
                              rc.comment)
            end
        end
    end
    ...
    _H.set_keymap({buf}, ds.modes, ds.shortcut, M.cmd.ChatDelete,
                  "GPT prompt Chat Delete")

    local ss = M.config.chat_shortcut_stop
    _H.set_keymap({buf}, ss.modes, ss.shortcut, M.cmd.Stop,
                  "GPT prompt Chat Stop")

    -- conceal parameters in model header so it's not distracting
    if M.config.chat_conceal_model_params then
        vim.opt_local.conceallevel = 2
        vim.opt_local.concealcursor = ""
        vim.fn.matchadd("Conceal", [[^- model: .*model.:.[^"]*\zs".*\ze]], 10,
                        -1, {conceal = "…"})
        vim.fn.matchadd("Conceal", [[^- model: \zs.*model.:.\ze.*]], 10, -1,
                        {conceal = "…"})
        vim.fn.matchadd("Conceal", [[^- role: .\{64,64\}\zs.*\ze]], 10, -1,
                        {conceal = "…"})
        vim.fn.matchadd("Conceal", [[^- role: .[^\\]*\zs\\.*\ze]], 10, -1,
                        {conceal = "…"})
    end
...
M.prep_context = function(buf, file_name)
    if not _H.ends_with(file_name, ".gp.md") then return end
...
M.open_buf = function(file_name, target, kind, toggle)
    ...
    if toggle then M._toggle_close(kind) end
    ...
    if target == M.BufTarget.popup then
        ...
        buf, win, close, _ = M._H.create_popup(old_buf, M._Name .. " Popup",
                                               function(w, h)
        ...
        if not toggle then
            M._toggle_add(M._toggle_kind.popup,
                          {win = win, buf = buf, close = close})
        end
    ...
    if not toggle then return buf end
...
M.new_chat = function(params, toggle, system_prompt, agent)
    ...
    while #stamp < 3 do stamp = "0" .. stamp end
    time = time .. "." .. stamp
    ...
    if params.range == 2 then M.append_selection(params, cbuf, buf) end
    M._H.feedkeys("G", "xn")
    return buf
end
...
M.cmd.ChatNew = function(params, system_prompt, agent)
    if agent then
        if not type(agent) == "table" or not agent.provider then
            M.warning("The `gp.cmd.ChatNew` method signature has changed.\n" ..
    ...
    if M._toggle_close(M._toggle_kind.chat) then
        ...
        if params.args == "" then params.args = M.config.toggle_target end
...
M.cmd.ChatToggle = function(params, system_prompt, agent)
    if M._toggle_close(M._toggle_kind.popup) then return end
    if M._toggle_close(M._toggle_kind.chat) and params.range ~= 2 then return end
    ...
    if params.range ~= 2 then
        ...
        if vim.fn.filereadable(last) == 1 then
            ...
            M.open_buf(last, M.resolve_buf_target(params), M._toggle_kind.chat,
                       true)
            return
        end
    end
...
M.cmd.ChatPaste = function(params)
    ...
    buf = win_found and buf or
              M.open_buf(last, target, M._toggle_kind.chat, true)
...
M.cmd.ChatDelete = function()
    ...
    vim.ui.input({prompt = "Delete " .. file_name .. "? [y/N] "},
                 function(input)
...
M.chat_respond = function(params, buffer, window)
    ...
    if reason then
        M.warning("File " .. vim.inspect(file_name) ..
                      " does not look like a chat file: " .. vim.inspect(reason))
        return
    end
    ...
    for _, line in ipairs(lines) do
        ...
        if key ~= nil then headers[key] = value end
    ...
    if header_end == nil then
        M.error(
            "Error while parsing headers: --- not found. Check your chat template.")
        return
    end
    ...
    ---@diagnostic disable-next-line: cast-local-type
    agent_suffix = M._H.template_render(agent_suffix,
                                        {["{{agent}}"] = agent_name})
    ...
    local last_content_line = M._H.last_content_line(buf)
    vim.api.nvim_buf_set_lines(buf, last_content_line, last_content_line, false,
                               {"", agent_prefix .. agent_suffix, ""})

    -- call the model and write response
    M.query(buf, headers.provider or agent.provider,
            M.prepare_payload(messages, headers.model or agent.model,
        ...
        M._H.undojoin(buf)
        vim.api.nvim_buf_set_lines(buf, last_content_line, last_content_line,
                                   false,
                                   {"", "", M.config.chat_user_prefix, ""})
        ...
        if headers.topic == "?" then
            ...
            table.insert(messages, {
                role = "user",
                content = M.config.chat_topic_gen_prompt
            })
            ...
            -- call the model
            M.query(nil, headers.provider or agent.provider,
                    M.prepare_payload(messages, headers.model or agent.model,
                                      headers.provider or agent.provider),
                    topic_handler, vim.schedule_wrap(function()
                -- get topic from invisible buffer
                local topic = vim.api
                                  .nvim_buf_get_lines(topic_buf, 0, -1, false)[1]
...
M.cmd.ChatRespond = function(params)
    ...
    if n_requests == nil or math.floor(n_requests) ~= n_requests or n_requests <=
        0 then
        M.warning(
            "args for ChatRespond should be a single positive number, not: " ..
                params.args)
        return
    end
    ...
    while cur_index > 0 and n_requests > 0 do
        if lines[cur_index]:sub(1, #M.config.chat_user_prefix) ==
            M.config.chat_user_prefix then n_requests = n_requests - 1 end
...
M.cmd.ChatFinder = function()
    ...
    local picker_buf, picker_win, picker_close, picker_resize = M._H
                                                                    .create_popup(
                                                                    nil,
    ...
    local preview_buf, preview_win, preview_close, preview_resize = M._H
                                                                        .create_popup(
                                                                        nil,
    ...
    vim.cmd("highlight default " .. hl_cursorline_group ..
                " gui=standout cterm=standout")
    ...
    local refresh = function()
        if not vim.api.nvim_buf_is_valid(picker_buf) then return end
        ...
        picker_match_id = vim.fn.matchadd(hl_search_group, regex, 0, -1,
                                          {window = picker_win})
        preview_match_id = vim.fn.matchadd(hl_search_group, regex, 0, -1,
                                           {window = preview_win})
        picker_pos_id = vim.fn.matchaddpos(hl_cursorline_group, {{index}}, 0,
                                           -1, {window = picker_win})
    end
        ...
        _H.grep_directory(nil, dir, cmd, function(results, re)
            ...
            for _, f in ipairs(results) do
                if f.line:len() > 0 then
                    ...
                    local fline = string.format("%s:%s %s", f.file:sub(3, -11),
                                                f.lnum, f.line)
    ...
    _H.autocmd({"TextChanged", "TextChangedI", "TextChangedP", "TextChangedT"},
               {command_buf}, function()
        vim.api.nvim_win_set_cursor(picker_win, {1, 0})
        refresh_picker()
    end, gid)

    -- close on buffer delete
    _H.autocmd({"BufWipeout", "BufHidden", "BufDelete"},
               {picker_buf, preview_buf, command_buf}, close, gid)

    -- close by escape key on any window
    _H.set_keymap({picker_buf, preview_buf, command_buf}, "n", "<esc>", close)
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n"}, "<C-c>",
                  close)
    ...
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n", "v"},
                  "<cr>", open_chat)
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n", "v"},
                  "<C-f>", function() open_chat(M.BufTarget.popup, false) end)
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n", "v"},
                  "<C-x>", function() open_chat(M.BufTarget.split, false) end)
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n", "v"},
                  "<C-v>", function() open_chat(M.BufTarget.vsplit, false) end)
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n", "v"},
                  "<C-t>", function() open_chat(M.BufTarget.tabnew, false) end)
    _H.set_keymap({picker_buf, preview_buf, command_buf}, {"i", "n", "v"},
                  "<C-g>", function()
    ...
    _H.set_keymap({command_buf, picker_buf}, {"i", "n"}, "<tab>", function()
        ...
        if next_index > #picker_files then next_index = 1 end
    ...
    _H.set_keymap({command_buf, picker_buf}, {"i", "n"}, "<s-tab>", function()
        local index = vim.api.nvim_win_get_cursor(picker_win)[1]
        local next_index = index - 1
        if next_index < 1 then next_index = #picker_files end
        vim.api.nvim_win_set_cursor(picker_win, {next_index, 0})
        refresh()
    end)
...
M.cmd.Agent = function(params)
    ...
    if agent_name == "" then
        M.info(
            " Chat agent: " .. M._state.chat_agent .. "  |  Command agent: " ..
                M._state.command_agent)
        return
    end
...
M.get_command_agent = function(name)
    ...
    if M.agents[name] == nil then
        M.warning("Command Agent " .. name .. " not found, using " ..
                      M._state.command_agent)
...
M.get_chat_agent = function(name)
    ...
    if M.agents[name] == nil then
        M.warning("Chat Agent " .. name .. " not found, using " ..
                      M._state.chat_agent)
...
M.cmd.Context = function(params)
    ...
    if params.range ~= 2 then
        if M._toggle_close(M._toggle_kind.context) then return end
    end
    ...
    if vim.fn.filereadable(file_name) ~= 1 then
        vim.fn.writefile({"Additional context is provided below.", ""},
                         file_name)
    end
...
M.Prompt = function(params, target, agent, template, prompt, whisper, callback)
    if not agent or not type(agent) == "table" or not agent.provider then
        M.warning("The `gp.Prompt` method signature has changed.\n" ..
                      "Please update your hook functions as demonstrated in the example below:\n\n" ..
                      examplePromptHook ..
                      "\nFor more information, refer to the 'Extend Functionality' section in the documentation.")
        return
    ...
    local cb = function(command)
        ...
        local on_exit = function(qid)
            ...
            if not M.config.command_auto_select_response then return end

            -- don't select popup response
            if target == M.Target.popup then return end
        ...
        local sys_prompt = M.template_render(agent.system_template, command,
                                             selection, filetype, filename)
        ...
        if repo_instructions ~= "" then
            table.insert(messages,
                         {role = "system", content = repo_instructions})
        end

        local user_prompt = M.template_render(template, command, selection,
                                              filetype, filename)
        ...
        local cursor = true
        if not M.config.command_auto_select_response then cursor = false end
        ...
        elseif target == M.Target.popup then
            ...
            buf, win, popup_close, _ = M._H.create_popup(nil, M._Name ..
                                                             " popup (close with <esc>/<C-c>)",
                                                         function(w, h)
                local top = M.config.style_popup_margin_top or 2
                local bottom = M.config.style_popup_margin_bottom or 8
                local left = M.config.style_popup_margin_left or 1
            ...
            M._toggle_add(M._toggle_kind.popup,
                          {win = win, buf = buf, close = popup_close})
        elseif type(target) == "table" then
            ...
            local group = M._H.create_augroup("GpScratchSave" .. _H.uuid(),
                                              {clear = true})
        ...
        M.query(buf, agent.provider,
                M.prepare_payload(messages, agent.model, agent.provider),
                handler, vim.schedule_wrap(function(qid)
            on_exit(qid)
            vim.cmd("doautocmd User GpDone")
        end), callback)
    end
...
M.Whisper = function(language, callback)
    ...
    local rec_options = {
        sox = {
            cmd = "sox",
            opts = {
                "-c", "1", "--buffer", "32", "-d", "rec.wav", "trim", "0",
                "3600"
            },
            exit_code = 0
        },
        arecord = {
            cmd = "arecord",
            opts = {
                "-c", "1", "-f", "S16_LE", "-r", "48000", "-d", 3600, "rec.wav"
            },
            exit_code = 1
        },
        ffmpeg = {
            cmd = "ffmpeg",
            opts = {
                "-y", "-f", "avfoundation", "-i", ":0", "-t", "3600", "rec.wav"
            },
            exit_code = 255
        }
    }
    ...
    -- create popup
    local buf, _, close_popup, _ = M._H.create_popup(nil, M._Name .. " Whisper",
                                                     function(w, h)
    ...
    timer:start(0, 200, vim.schedule_wrap(function()
        ...
        counter = counter + 1
        if counter % 22 == 0 then counter = 0 end
    end))
    ...
    _H.set_keymap({buf}, {"n", "i", "v"}, "<esc>", function() M.cmd.Stop() end)

    _H.set_keymap({buf}, {"n", "i", "v"}, "<C-c>", function() M.cmd.Stop() end)

    local continue = false
    _H.set_keymap({buf}, {"n", "i", "v"}, "<cr>", function()
        continue = true
        vim.defer_fn(function() M.cmd.Stop() end, 300)
    end)
    ...
    local transcribe = function()
        local cmd = "cd " .. M.config.whisper_dir .. " && " ..
                        "export LC_NUMERIC='C' && " -- normalize volume to -3dB
        .. "sox --norm=-3 rec.wav norm.wav && " -- get RMS level dB * silence threshold
        .. "t=$(sox 'norm.wav' -n channels 1 stats 2>&1 | grep 'RMS lev dB' " ..
                        " | sed -e 's/.* //' | awk '{print $1*" ..
                        M.config.whisper_silence .. "}') && " -- remove silence, speed up, pad and convert to mp3
        ..
                        "sox -q norm.wav -C 196.5 final.mp3 silence -l 1 0.05 $t'dB' -1 1.0 $t'dB'" ..
                        " pad 0.1 0.1 tempo " .. M.config.whisper_tempo ..
                        " && " -- call openai
        .. curl ..
                        " --max-time 20 https://api.openai.com/v1/audio/transcriptions -s " ..
                        '-H "Authorization: Bearer ' .. M.config.openai_api_key ..
                        '" -H "Content-Type: multipart/form-data" ' ..
                        '-F model="whisper-1" -F language="' .. language ..
                        '" -F file="@final.mp3" ' .. '-F response_format="json"'

        M._H.process(nil, "bash", {"-c", cmd}, function(code, signal, stdout, _)
            if code ~= 0 then
                M.error(string.format("Whisper query exited: %d, %d", code,
                                      signal))
                return
            end
            ...
            if callback and stdout then callback(text) end
        end)
    end
    ...
    if not rec_cmd then
        ...
        if vim.fn.executable("ffmpeg") == 1 then
            local devices = vim.fn.system(
                                "ffmpeg -devices -v quiet | grep -i avfoundation | wc -l")
            devices = string.gsub(devices, "^%s*(.-)%s*$", "%1")
            if devices == "1" then rec_cmd = "ffmpeg" end
        end
        if vim.fn.executable("arecord") == 1 then rec_cmd = "arecord" end
    end
    ...
    elseif type(rec_cmd) == "string" and rec_options[rec_cmd] then
        ...
        M.error(string.format("Whisper got invalid recording command: %s",
                              rec_cmd))
        close()
        return
    end
    for i, v in ipairs(cmd.opts) do
        if v == "rec.wav" then cmd.opts[i] = rec_file end
    end
    ...
    M._H.process(nil, cmd.cmd, cmd.opts, function(code, signal, stdout, stderr)
        ...
        if code and code ~= cmd.exit_code then
            M.error(
                cmd.cmd .. " exited with code and signal:\ncode: " .. code ..
                    ", signal: " .. signal .. "\nstdout: " ..
        ...
        vim.schedule(function() transcribe() end)
    end)
end
...
M.cmd.Whisper = function(params)
    ...
    if args[1] ~= "" then language = args[1] end
...
M.get_image_agent = function()
    ...
    local cmd_prefix = M._H.template_render(template, {
        ["{{agent}}"] = M._state.image_agent
    })
...
M.cmd.Image = function(params)
    ...
    if prompt == "" then
        vim.ui.input({prompt = agent.cmd_prefix}, function(input)
            ...
            M.generate_image(prompt, agent.model, agent.quality, agent.style,
                             agent.size)
        end)
    else
        M.generate_image(prompt, agent.model, agent.quality, agent.style,
...
function M.generate_image(prompt, model, quality, style, size)
    ...
    local args = {
        "-s", "-H", "Content-Type: application/json", "-H",
        "Authorization: Bearer " .. M.config.openai_api_key, "-d",
        vim.json.encode(payload), "https://api.openai.com/v1/images/generations"
    }
    ...
    _H.process(nil, cmd, args, function(code, signal, stdout_data, stderr_data)
        ...
        if code ~= 0 then
            M.error("Image generation exited: code: " .. code .. " signal: " ..
                        signal .. " stdout: " .. stdout_data .. " stderr: " ..
                        stderr_data)
            return
        end
        ...
        if result and result.data and result.data[1] and result.data[1].url then
            ...
            vim.ui.input({
                prompt = M.config.image_prompt_save,
                completion = "file",
                default = M.config.image_dir
            }, function(save_path)
                ...
                _H.process(nil, "curl", {"-s", "-o", save_path, image_url},
                           function(save_code, save_signal, save_stdout_data,
                                    save_stderr_data)
                    M.spinner.stop_spinner()
                    ...
                    if save_code == 0 then
                        ...
                        M.error("Failed to save image: path: " .. save_path ..
                                    " code: " .. save_code .. " signal: " ..
                                    save_signal .. " stderr: " ..
                                    save_stderr_data)
                    end
                end)
